...
class HostingCustomer extends Authenticatable
{
    ...
    public function fetchHost($request, $columns) {
      
        $query = HostingCustomer::orderBy('id', 'desc');
        ...
        if (isset($request['search']['value'])) {
            $query->where(function ($q) use ($request) {
                ...
                $q->where('amount', 'like', '%' . $request['search']['value'] . '%');
            });
        }
    ...
    {
        return $this->belongsTo(Invoice::class,'project_id');
    }
}
